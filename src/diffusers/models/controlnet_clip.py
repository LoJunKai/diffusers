...
class CLIPWrapper:
    ...
    (from src/transformers/models/clip/modeling_clip.py - CLIPVisionEmbeddings)
        def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
            ...
            patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))    # shape = [*, self.embed_dim, 16, 16]
            patch_embeds = patch_embeds.flatten(2).transpose(1, 2)                      # shape = [*, 256, self.embed_dim]
            class_embeds = self.class_embedding.expand(batch_size, 1, -1)               # shape = [*, 1, self.embed_dim]
            embeddings = torch.cat([class_embeds, patch_embeds], dim=1)                 # class_embeds added as first of 256.
    ...
    def preprocess_image(self, image):
        # Convert image to image embeddings reshaped to (1024 16, 16).
        last_hidden_state = self.get_img_embed(image)
